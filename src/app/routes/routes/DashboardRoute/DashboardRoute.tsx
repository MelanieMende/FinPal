...
export default function DashboardRoute() {
	...
	// Basic calculations
	const totalAssetsValue = assets.reduce((acc: number, asset) => acc + (asset.current_shares * (asset.price || 0)), 0);
	const totalCash = cash.reduce((acc: number, c) => acc + c.amount, 0);
	const netWorth = totalAssetsValue + totalCash;

	// Chart Data: Allocation
	const allocationData = [
		...assets
			.filter(a => a.current_shares > 0)
			.map(a => ({
				name: a.name,
				value: a.current_shares * (a.price || 0)
			})),
		...
		<div id="DashboardRoute" className="w-full max-w-7xl mx-auto p-4">
			...
			<div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
				...
				<Card className="glass-card flex flex-col justify-between">
					...
					<div className="text-3xl font-bold text-white tracking-tight">
						{new Intl.NumberFormat('de-DE', { style: 'currency', currency: 'EUR' }).format(totalAssetsValue)}
					</div>
					<div className="mt-4 text-sm text-indigo-300 font-medium">
						{assets.filter(a => a.current_shares > 0).length} active positions
					</div>
				</Card>
				...
				<Card className="glass-card flex flex-col justify-between">
					...
					<div className="text-3xl font-bold text-white tracking-tight">
						{new Intl.NumberFormat('de-DE', { style: 'currency', currency: 'EUR' }).format(totalCash)}
					</div>
					<div className="mt-4 text-sm text-emerald-300 font-medium">
						{((totalCash / netWorth) * 100).toFixed(1)}% of total portfolio
					</div>
				</Card>
			</div>
			...
			<div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
				...
				<Card className="glass-card h-[400px] flex flex-col">
					...
					<div className="overflow-auto scrollbar-hide">
						{assets
							.filter(a => a.current_shares > 0)
							.sort((a, b) => (b.current_shares * (b.price || 0)) - (a.current_shares * (a.price || 0)))
							.slice(0, 5)
							.map((asset, index) => (
								<div key={asset.ID} className="flex justify-between items-center py-3 border-b border-white/5 last:border-0 grow">
									...
									<div className="text-right">
										<div className="font-bold text-sm">
											{new Intl.NumberFormat('de-DE', { style: 'currency', currency: 'EUR' }).format(asset.current_shares * (asset.price || 0))}
										</div>
										<div className="text-xs text-green-400">
											{((asset.current_shares * (asset.price || 0) / netWorth) * 100).toFixed(1)}%
										</div>
									</div>
								</div>
							))}
