    ...
    <tr id={"TransactionListItem_" + props.transaction.ID} className="border-b border-white/5 last:border-0 hover:bg-white/5 transition-colors group">
			...
			<TableCell className="p-3">
				<input 
					...
					className="glass-input-minimal text-xs bg-transparent border-0 focus:ring-1 focus:ring-blue-500/50 rounded px-1 transition-all"
				/>
			</TableCell>
      ...
      <TableCell className="p-3">
				<select 
					...
					onBlur={() => validateAndSave()}
					className="glass-input-minimal text-sm font-semibold bg-transparent border-0 text-white w-full truncate focus:ring-1 focus:ring-blue-500/50 rounded px-1"
				>
      ...
      <TableCell className="p-3 text-right">
				<input 
					...
					onBlur={() => validateAndSave()}
					className="text-right glass-input-minimal text-sm font-bold bg-transparent border-0 text-white w-20 focus:ring-1 focus:ring-blue-500/50 rounded px-1"
				/>
			</TableCell>

			{/* Shares Cumulated */}
      <TableCell className="p-3 text-right text-gray-500 font-medium text-xs">
				{shareFormatter.format(props.transaction.shares_cumulated || 0)}
			</TableCell>
			...
			{/* Fees / Tax */}
			<TableCell className="p-3 text-right">
				<div className="text-xs group-hover:text-gray-300 transition-colors">
					{euroFormatter.format(props.transaction.fee || 0)}
				</div>
				<div className="text-[10px] text-gray-500 italic">
					+ {euroFormatter.format(props.transaction.solidarity_surcharge || 0)} Tax
				</div>
			</TableCell>

			{/* Invest Cumulated */}
			<TableCell className="p-3 text-right text-gray-500 text-xs italic">
				{euroFormatter.format(props.transaction.invest_cumulated || 0)}
			</TableCell>

			{/* In-/Outcome */}
			<TableCell className="p-3 text-right">
				<div className={`text-sm font-bold ${props.transaction.in_out > 0 ? 'text-emerald-400' : 'text-red-400'}`}>
					{props.transaction.in_out > 0 ? "+" : ""}{euroFormatter.format(props.transaction.in_out || 0)}
				</div>
			</TableCell>
